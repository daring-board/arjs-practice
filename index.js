...
async function main(video){
    ...
    while(true) {
        ...
        if (nose.score > 0.8) {
            ...
            const term = 180;
            const ctr_x = Math.sin(Math.PI * rotate.y / term) * Math.cos(Math.PI * rotate.x / term) + 0.8;
            const ctr_y = Math.sin(Math.PI * rotate.y / term) * Math.sin(Math.PI * rotate.x / term) + 0.8;
